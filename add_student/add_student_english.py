...
class AddStudent_English120(ctk.CTkFrame):
    ...
    def add_student(self):
        ...
        self.cursor.execute("select student_ID from enrollment where course_ID = 3 and student_ID = %s", (student,))
        is_enrolled = self.cursor.fetchall()
        if len(is_enrolled) == 0:
            ## get student count and ensure it is beneath the max amount
            self.cursor.execute("select student_ID from gradebook where course_ID = 3")
            ...
            if len(students) > self.max_students:
                ...
                self.cursor.execute("insert into enrollment values (%s, 3)", (student,))
                self.mydb.commit()
